<template>
  <el-container class="home-app">
    <el-aside class="home-aside" :width=" Collapse?  '60px':'200px'">
      <div class="logo" :class="{cloes : Collapse}"></div>
       <!-- $route.path 拿当前路由路径不包括# -->
      <el-menu
        :default-active="$route.path"
        background-color="#002033"
        text-color="#fff"
        active-text-color="#ffd04b"
        style="border-right:none"
        :collapse="Collapse"
        :collapse-transition="false"
        router
      >
        <el-menu-item index="/">
          <i class="el-icon-s-home"></i>
          <span slot="title">首页</span>
        </el-menu-item>
        <el-menu-item index="/article">
          <i class="el-icon-document"></i>
          <span slot="title">内容管理</span>
        </el-menu-item>
        <el-menu-item index="/image">
          <i class="el-icon-picture"></i>
          <span slot="title">素材管理</span>
        </el-menu-item>
        <el-menu-item index="/publish">
          <i class="el-icon-s-promotion"></i>
          <span slot="title">发布文章</span>
        </el-menu-item>
        <el-menu-item index="/comment">
          <i class="el-icon-chat-dot-round"></i>
          <span slot="title">评论管理</span>
        </el-menu-item>
        <el-menu-item index="/fans">
          <i class="el-icon-present"></i>
          <span slot="title">粉丝管理</span>
        </el-menu-item>
        <el-menu-item index="/setting">
          <i class="el-icon-setting"></i>
          <span slot="title">个人设置</span>
        </el-menu-item>
      </el-menu>
    </el-aside>
    <el-container>
      <el-header class="home-header">
        <span class="el-icon-s-fold" @click="togglemenu"></span>
        <span class="text">江苏传智播客有限公司</span>
        <el-dropdown style="float: right">
          <span class="el-dropdown-link">
            <img
              style=" vertical-align: middle;width:30px; margin-right: 5px;"
              :src="avater"
              alt
            />
            <b>{{name}}</b>
            <i class="el-icon-arrow-down el-icon--right"></i>
          </span>
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item class="el-icon-setting" @click.native="setting()">个人设置</el-dropdown-item>
            <el-dropdown-item class="el-icon-user-solid" @click.native="login()">退出登录</el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
      </el-header>
      <el-main>
        <router-view></router-view>
      </el-main>
    </el-container>
  </el-container>
</template>

<script>
//  组件绑事件 需要用修饰符  组件默认不支持dom事件
export default {
  name: '',
  data () {
    return {
      Collapse: false,
      name: '',
      avater: ''
    }
  },
  created () {
    const user = JSON.parse(window.sessionStorage.getItem('toutiao'))
    this.name = user.name
    this.avater = user.photo
  },
  methods: {
    togglemenu () {
      this.Collapse = !this.Collapse
    },
    login () {
      window.sessionStorage.removeItem('toutiao')
      this.$router.push('/login')
    },
    setting () {
      this.$router.path('/setting')
    }
  }
}
</script>

<style lang="less"  scoped>
.home-app {
  width: 100%;
  height: 100%;
  position: absolute;
  left: 0;
  top: 0;
  .home-aside {
    background-color: #002033;
    .logo {
      height: 60px;
      background: #002048 url(../../assets/images/logo_admin.png) no-repeat
        center / 100px auto;
    }
    .cloes {
      background: url(../../assets/images/logo_admin_01.png) no-repeat center /
        40px auto;
    }
  }
  .home-header {
    border-bottom: 1px solid #ddd;
    line-height: 60px;
    .el-icon-s-fold {
      font-size: 26px;
      vertical-align: middle;
    }
    .text {
      vertical-align: middle;
    }
  }
}
</style>
